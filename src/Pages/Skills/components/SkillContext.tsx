...
const stateMachine = (state: SkillContextValue, action: any) => {
  // filters
  ...
  // skill description
  const updateSkillDescription = (visibility: boolean, skill?: SkillIds) => ({
    ...state,
...
export const SkillContextProvider = (props: {
  ...
  const handleSort = useCallback(
    (sort: SkillSorts) => dispatch({ type: SkillActions.Sort, sort }),
    [dispatch]
  );

  // skill description panel
  const setSkillDescription = useCallback(
    (skill: SkillIds) => dispatch({ type: SkillActions.SetSkill, skill }),
    [dispatch]
  );
  ...
  const searchSkills = useCallback(
    (value: string) => dispatch({ type: SkillActions.Search, value }),
    [dispatch]
  );
...
export const useSkillContext = (): SkillContextValue => {
  ...
  if (context === undefined) {
    throw new Error("useSkillContext must be used within SkillContextProvider");
  }
  return context;
};
export const useSkillContextDispatch = () => {
  ...
  if (context === undefined) {
    throw new Error(
      "useSkillContextDispatch must be used within SkillContextDispatch.Provider"
    );
  }
  return context;
};
